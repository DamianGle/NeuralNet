...
namespace BPN
{
    NetworkTrainer::NetworkTrainer( Settings const& settings, Network* pNetwork )
    	...
    	, LoggerFileName(settings.LoggerFileName)
        , m_currentEpoch( 0 )
    ...
    void NetworkTrainer::Train( TrainingData const& trainingData, bool mode )
    {
        ...
        if(!mode)
			{
				file.open( LoggerFileName, std::ios::in | std::ios::trunc | std::ios::out );
				...
				if( file.good() == true )
					{
						file << std::endl << " Neural Network Training Starting: " << std::endl
							 << " LR: " << m_learningRate << ", Momentum: " << m_momentum << ", Max Epochs: " << m_maxEpochs << std::endl
							 << " " << m_pNetwork->m_numInputs<< " Input Neurons, " << m_pNetwork->m_numHidden << " Hidden Neurons, " << m_pNetwork->m_numOutputs<< " Output Neurons" << std::endl << std::endl;
					}
			}
            ...
            if(!mode)
				{
					...
					if( file.good() == true )
						{
							file << "Epoch: " << m_currentEpoch;
							file << " TSA: " << m_trainingSetAccuracy << "%, MSE: " << m_trainingSetMSE;
							file << " GSA: " << m_generalizationSetAccuracy << "%, MSE: " << m_generalizationSetMSE << std::endl;
						}
				}
            m_currentEpoch++;
        }
        ...
        if(!mode)
        	{
        		...
        		if( file.good() == true )
					{
						file << std::endl << std::endl << "Training Complete!!! - > Elapsed Epochs: " << m_currentEpoch << std::endl;
						file << " Validation Set Accuracy: " << m_validationSetAccuracy << std::endl;
						file << " Validation Set MSE: " << m_validationSetMSE << std::endl << std::endl;
